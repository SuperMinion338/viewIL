  ...
  return (
    ...
    >
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/3 left-1/4 w-96 h-96 rounded-full bg-blue-600 opacity-10 blur-3xl animate-blob" />
        <div className="absolute bottom-1/3 right-1/4 w-80 h-80 rounded-full bg-indigo-500 opacity-10 blur-3xl animate-blob animation-delay-4000" />
      </div>
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 mt-2 text-sm">הצטרף ל-2,000+ יוצרים ישראלים 🇮🇱</p>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div>
              ...
              {password.length > 0 && (
                <div className={`flex items-center gap-1.5 mt-1.5 text-xs ${passwordStrong ? "text-green-400" : "text-slate-400"}`}>
                  <Check className="w-3 h-3" />
                  {passwordStrong ? "סיסמה תקינה" : "לפחות 6 תווים"}
                </div>
              )}
            </div>
